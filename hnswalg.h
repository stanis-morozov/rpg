...
namespace hnswlib {
  ...
  public:
    ...
    HierarchicalNSW(SpaceInterface<dist_t> *s, const string &location) {
    ...
    std::priority_queue< std::pair< dist_t, tableint  >> searchBaseLayer(tableint ep, void *datapoint, int layer) {
      ...
      dist_t dist = constructionDistance(datapoint, getDataByInternalId(ep));
      ...
      while (!candidateSet.empty()) {
        ...
        for (int j = 0; j < size; j++) {
          ...
          if (!(massVisited[tnum] == currentV)) {
            ...
            dist_t dist = constructionDistance(datapoint, currObj1);
    ...
    std::priority_queue< std::pair< dist_t, tableint  >, vector<pair<dist_t, tableint>>, CompareByFirst> searchBaseLayerST(tableint ep, void *datapoint, size_t ef) {
      ...
      dist_t dist = searchDistance(datapoint, getDataByInternalId(ep));
      dist_calc++;
      topResults.emplace(dist, ep);
      candidateSet.emplace(-dist, ep);
      massVisited[ep] = currentV;
      ...
      while (!candidateSet.empty()) {
        ...
        for (int j = 1; j <= size; j++) {
          ...
          if (!(massVisited[tnum] == currentV)) {
            ...
            char *currObj1 = (getDataByInternalId(tnum));
            dist_t dist = searchDistance(datapoint, currObj1);
            dist_calc++;
    ...
    void getNeighborsByHeuristic2(std::priority_queue< std::pair< dist_t, tableint>> &topResults, const int NN)
    {
      ...
      /*for (int i = 0; i < NN; i++) {
        topResults.emplace(-resultSet.top().first, resultSet.top().second);
        resultSet.pop();
      }
      ...
      while (resultSet.size()) {
        ...
        for (std::pair< dist_t, tableint> curen2 : returnlist) {
          dist_t curdist =
            constructionDistance(getDataByInternalId(curen2.second), getDataByInternalId(curen.second));;
    ...
    void mutuallyConnectNewElement(tableint cur_c, std::priority_queue< std::pair< dist_t, tableint  >> topResults, int level) {
      ...
      }
      {
        ...
        *ll_cur = rez.size();
        ...
        for (int idx = 0; idx < (int)rez.size(); idx++) {
          if (data[idx])
            throw runtime_error("Should be blank");
      ...
      for (int idx = 0; idx < (int)rez.size(); idx++) {
        ...
        if ((int)sz_link_list_other > (int)Mcurmax || sz_link_list_other < 0)
          throw runtime_error("Bad sz_link_list_other");

        if ((int)sz_link_list_other < (int)Mcurmax) {
          ...
          *ll_other = sz_link_list_other + 1;
          ...
          tableint *data = (tableint *)(ll_other + 1);
          dist_t d_max = constructionDistance(getDataByInternalId(cur_c), getDataByInternalId(rez[idx]));
          // Heuristic:
          std::priority_queue< std::pair< dist_t, tableint>> candidates;
          candidates.emplace(d_max, cur_c);

          for (int j = 0; j < sz_link_list_other; j++) {
            candidates.emplace(constructionDistance(getDataByInternalId(data[j]), getDataByInternalId(rez[idx])), data[j]);
          }
    ...
    void addPoint(void *datapoint, labeltype label, int level = -1) {
      ...
      if ((int)currObj != -1) {
        ...
        if (curlevel < maxlevelcopy) {
          ...
          dist_t curdist = constructionDistance(datapoint, getDataByInternalId(currObj));
          for (int level = maxlevelcopy; level > curlevel; level--) {
            ...
            while (changed) {
              ...
              for (int i = 0; i < size; i++) {
                ...
                dist_t d = constructionDistance(datapoint, getDataByInternalId(cand));
                if (d < curdist) {
                  curdist = d;
                  currObj = cand;
                  changed = true;
        ...
        for (int level = min(curlevel, maxlevelcopy); level >= 0; level--) {
          ...
          mutuallyConnectNewElement(cur_c, topResults, level);
        }
    ...
    std::priority_queue< std::pair< dist_t, labeltype >> searchKnn(void *query_data, int k) {
      tableint currObj = enterpoint_node;
      dist_t curdist = searchDistance(query_data, getDataByInternalId(enterpoint_node));
      dist_calc++;
      for (int level = maxlevel_; level > 0; level--) {
        ...
        while (changed) {
          ...
          for (int i = 0; i < size; i++) {
            ...
            if (cand<0 || cand>maxelements_)
              throw runtime_error("cand error");
            dist_t d = searchDistance(query_data, getDataByInternalId(cand));
            dist_calc++;
            if (d < curdist) {
              curdist = d;
              currObj = cand;
    ...
    std::priority_queue< std::pair< dist_t, tableint>> searchKnnInternal(void *query_data, int k) {
      tableint currObj = enterpoint_node;
      dist_t curdist = constructionDistance(query_data, getDataByInternalId(enterpoint_node));
      dist_calc++;
      for (int level = maxlevel_; level > 0; level--) {
        bool changed = true;
        while (changed) {
          ...
          for (int i = 0; i < size; i++) {
            ...
            if (cand<0 || cand>maxelements_)
              throw runtime_error("cand error");
            dist_t d = constructionDistance(query_data, getDataByInternalId(cand));
            dist_calc++;
            if (d < curdist) {
              curdist = d;
              currObj = cand;
